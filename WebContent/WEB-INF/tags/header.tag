<%@tag language="java" pageEncoding="UTF-8"%><%--
--%><%@tag body-content="empty"%><%--
--%><div id="header">
	<a href="http://${ initParam['websiteBaseURL'] }"><img src="http://${ initParam['websiteBaseURL'] }/images/heading.png" alt="Browserprint"></a>
</div>
<div>
	<a href="http://${ initParam['websiteBaseURL'] }">Home</a> |
	<a href="http://${ initParam['websiteBaseURL'] }/view">View fingerprints</a> |
	<a href="http://${ initParam['websiteBaseURL'] }/statistics">Statistics</a> |
	<a href="http://${ initParam['websiteBaseURL'] }/blog/">Blog</a>
</div>